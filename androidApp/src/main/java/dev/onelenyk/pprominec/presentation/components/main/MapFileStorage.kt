...
class MapFileStorage(private val context: Context) {
    ...
    private suspend fun getOrCreateMapFilesDir(): File = withContext(Dispatchers.IO) {
    ...
    suspend fun copyInputStreamToStorage(inputStream: InputStream, fileName: String): File = withContext(Dispatchers.IO) {
    ...
    suspend fun copyUriToStorage(uri: Uri, fileName: String): File? {
        val inputStream = context.contentResolver.openInputStream(uri) ?: return null
        return copyInputStreamToStorage(inputStream, fileName)
    }

    // Remove a file from storage by name
    suspend fun removeFromStorage(fileName: String): Boolean = withContext(Dispatchers.IO) {
        File(getOrCreateMapFilesDir(), fileName).delete()
    }

    // Clear all files in the storage directory
    suspend fun clearStorage() = withContext(Dispatchers.IO) {
