...
class MapFilesRepository(private val context: Context) {
    ...
    fun getMapUris(): Flow<Set<String>> = context.mapFilesDataStore.data.map { prefs -> prefs[MAP_URIS_KEY] ?: emptySet() }
    ...
    fun getSelectedMapUri(): Flow<String?> = context.mapFilesDataStore.data.map { prefs -> prefs[SELECTED_MAP_URI_KEY] }
}
